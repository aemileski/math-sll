/*
 * $Id: math-sll.c,v 1.10 2002/06/17 13:58:33 andrewm Exp $
 *
 * Purpose
 ...
 *
 *	X = A * B
 *	  = (R1 * 2^0 + Q1 * 2^-32) * (R2 * 2^0 + Q2 * 2^-32)
 *	  = (R1 * 2^0) * (R2 * 2^0) + (R1 * 2^0) * (Q2 * 2^-32)
 *	  + (Q1 * 2^-32) * (R2 * 2^0) + (Q1 * 2^-32) * (Q2 * 2^-32)
 *	  = (R1 * R2) * 2^0 + (R1 * Q2) * 2^-32
 *	  + (Q1 * R2) * 2^-32 + (Q1 * Q2) * 2^-64
 * The last sum can be an unsigned product as there's no R1 or R2 involved.
 */
#if defined(__arm__)
